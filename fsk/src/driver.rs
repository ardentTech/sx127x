...
impl<DM: DataMode, SPI: SpiDevice> Sx127xFsk<DM, SPI> {
    ...
    pub async fn configure_band_specific_registers(&mut self) -> Result<(), Sx127xError<SPI::Error>> {
        ...
        if frequency >= LF_MAX_HZ && frequency <= HF_MIN_HZ {
            return Ok(()) // nop if out of range
        }

        let mut byte = self.read(OP_MODE).await?;
        set_bits(&mut byte, (frequency < LF_MAX_HZ) as u8, OP_MODE_LOW_FREQUENCY_MODE_ON_MASK, OP_MODE_LOW_FREQUENCY_MODE_ON_OFFSET);
    ...
    pub async fn packet_config(&mut self) -> Result<PacketConfig, Sx127xError<SPI::Error>> {
        ...
        Ok(PacketConfig {
            ...
            beacon_on: get_bits(config_2, PACKET_CONFIG_2_BEACON_ON_MASK, PACKET_CONFIG_2_BEACON_ON_OFFSET) == 1,
            crc_auto_clear_off: get_bits(config_1, PACKET_CONFIG_1_CRC_AUTO_CLEAR_OFF_MASK, PACKET_CONFIG_1_CRC_AUTO_CLEAR_OFF_OFFSET) == 0,
            crc_on: get_bits(config_1, PACKET_CONFIG_1_CRC_ON_MASK, PACKET_CONFIG_1_CRC_ON_OFFSET) == 1,
            crc_whitening_type: CrcWhiteningType::from(get_bits(config_1, PACKET_CONFIG_1_CRC_WHITENING_TYPE_MASK, PACKET_CONFIG_1_CRC_WHITENING_TYPE_OFFSET)),
    ...
    pub async fn set_packet_config(&mut self, config: PacketConfig) -> Result<(), Sx127xError<SPI::Error>> {
        ...
        set_bits(&mut config1, config.packet_format as u8, PACKET_CONFIG_1_PACKET_FORMAT_MASK, PACKET_CONFIG_1_PACKET_FORMAT_OFFSET);
        set_bits(&mut config1, config.dc_free as u8, PACKET_CONFIG_1_DC_FREE_MASK, PACKET_CONFIG_1_DC_FREE_OFFSET);
        set_bits(&mut config1, config.crc_on as u8, PACKET_CONFIG_1_CRC_ON_MASK, PACKET_CONFIG_1_CRC_ON_OFFSET);
    ...
    pub async fn transmit(&mut self, payload: &[u8]) -> Result<(), Sx127xError<SPI::Error>> {
        // TODO sleep/stdby mode?

        // if fixed packet format, payload is limited to 2047 bytes
        // if len payload > FIFO_CAPACITY, loop {}
        let len = payload.len();
        ...
        // FifoFull is set when FIFO contains 66 bytes (64 + CRC?) (64 + Addr + Len?)
