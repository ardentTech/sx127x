...
async fn main(spawner: Spawner) {
    ...
    let mut sx127x = Sx127xLora::new(spi_dev, example_config()).await.unwrap();
    sx127x.config_tx(TxConfig::new(OCP::default(), 20, PowerRamp::default(), false).unwrap()).await.unwrap();
